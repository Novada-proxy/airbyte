...
GitHub answers `502 Bad Gateway` or `504 Gateway Timeout` on the GraphQL endpoint when a query is too expensive to resolve within its deadline, which happens on the six GraphQL streams (`releases`, `projects_v2`, `pull_request_stats`, `reviews`, `issue_reactions` and `pull_request_comment_reactions`) for repositories with very large or heavily nested objects. As of version 2.7.0 the connector halves the page size and re-requests the same page — 10, 5, 2, then 1 records per page from the default — and, once the page size is 1, retries that page three more times with a growing wait, about two minutes in total. The reduced page size stays in effect for the rest of that repository and does not affect the page size used for other repositories. If GitHub still answers 502 or 504 after that, the stream fails with a message that names the starting page size. The deprecated `page_size_for_large_streams` setting, no longer shown in the UI but still honored when set through the API or Terraform, controls that starting page size (default 10); a lower value makes each query cheaper. A more recent **Start date** does not help here, because these streams filter by date after GitHub has returned the page.
